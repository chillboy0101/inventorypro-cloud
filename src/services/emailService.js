 ...
 */
export const generateEmailTemplate = (type, details) => {
    const templates = {
        low_stock: `
    ...
    `,
      ...
      <div style="font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto; padding: 20px; border: 1px solid #e5e7eb; border-radius: 8px;">
        ...
        <p>Status: <span style="font-weight: bold; color: ${details.status === 'delivered' ? '#10b981' :
            details.status === 'shipped' ? '#6366f1' :
                details.status === 'processing' ? '#f59e0b' :
                    details.status === 'cancelled' ? '#ef4444' : '#6b7280'};">${details.status.toUpperCase()}</span></p>

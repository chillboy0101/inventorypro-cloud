...
const InventoryTrends: React.FC = () => {
  ...
  const handleNewStockAdjustment = (adjustment: any) => {
    ...
    if (product) {
      ...
      setStockLevels(prev => {
        const updated = [...prev, newStockLevel].sort((a, b) => 
  ...
  const fetchStockLevels = async () => {
    try {
      ...
      if (adjustments) {
        ...
        }).filter((level): level is StockLevel => level !== null);

        allLevels = [...adjustmentLevels, ...currentLevels];
      }

      // Group by appropriate time interval based on selected period
      const groupedLevels = allLevels.reduce((acc: { [key: string]: StockLevel }, level) => {
        let key;
  ...
  const getProductColor = (productId: string, alpha: number = 1) => {
    const colors = [
      [255, 99, 132],   // Red
      [54, 162, 235],   // Blue
      [255, 206, 86],   // Yellow
    ...
    <div className="space-y-4">
      <div className="flex items-center justify-between">
        ...
        <select
          ...
          className="border border-gray-300 rounded-md px-3 py-1.5 text-sm focus:ring-indigo-500 focus:border-indigo-500 bg-white shadow-sm"
        >
